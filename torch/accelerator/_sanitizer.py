...
r"""
This module introduces Accelerator Sanitizer, a tool for detecting synchronization
errors between kernels run on different accelerator streams.
...
:func:`enable_accelerator_sanitizer()` or by exporting the
``TORCH_ACCELERATOR_SANITIZER`` environment variable.
"""
...
class SynchronizationError(Exception):
    """Base class for errors detected by Accelerator Sanitizer."""
...
class AcceleratorSanitizerErrors(Exception):
    """Wrapper class for errors reported by Accelerator Sanitizer."""
...
class TensorInfo:
    ...
    Args:
        allocation_stack_trace: the stack summary object captured during tensor
            allocation. Can be ``None`` if the allocation wasn't caught.
...
class EventHandler:
    """Analyzes sanitizer trace for synchronization errors.
...
def _resolve_device_type(device: _device_t = None) -> str:
    if device is None:
        acc = torch.accelerator.current_accelerator(check_available=True)
        if acc is None:
            raise RuntimeError(
                "Accelerator Sanitizer requires an available accelerator."
            )
        return acc.type
...
def _get_backend_module(device_type: str) -> ModuleType:
    return torch.get_device_module(device_type)


def _get_gpu_trace_module(device_type: str) -> ModuleType:
    backend_module = _get_backend_module(device_type)
    gpu_trace = getattr(backend_module, "_gpu_trace", None)
    if gpu_trace is not None:
        return gpu_trace

    module_name = f"torch.{device_type}._gpu_trace"
    try:
    ...
    raise RuntimeError(
        f"Accelerator Sanitizer for device type '{device_type}' requires "
        f"`torch.{device_type}._gpu_trace`, but that backend does not expose one."
    )


def _register_trace_callbacks(gpu_trace: ModuleType, event_handler: EventHandler) -> None:
    gpu_trace.register_callback_for_event_creation(
        event_handler._handle_event_creation
    )
    gpu_trace.register_callback_for_event_deletion(
        event_handler._handle_event_deletion
    )
    gpu_trace.register_callback_for_event_record(event_handler._handle_event_record)
    gpu_trace.register_callback_for_event_wait(event_handler._handle_event_wait)
    gpu_trace.register_callback_for_memory_allocation(
        event_handler._handle_memory_allocation
    )
    gpu_trace.register_callback_for_memory_deallocation(
        event_handler._handle_memory_deallocation
    )
    gpu_trace.register_callback_for_stream_creation(
        event_handler._handle_stream_creation
    )
    gpu_trace.register_callback_for_device_synchronization(
        event_handler._handle_device_synchronization
    )
    gpu_trace.register_callback_for_stream_synchronization(
        event_handler._handle_stream_synchronization
    )
    gpu_trace.register_callback_for_event_synchronization(
        event_handler._handle_event_synchronization
    )


def _get_current_stream(device_type: str) -> torch.Stream:
    device_module = _get_backend_module(device_type)
    if hasattr(device_module, "current_stream"):
        return device_module.current_stream()
    ...
    raise RuntimeError(
        f"Accelerator Sanitizer for device type '{device_type}' requires a "
        "`current_stream` implementation."
    )


def _get_stream_id(stream: torch.Stream) -> StreamId:
    if hasattr(stream, "native_handle"):
        return stream.native_handle
    if hasattr(stream, "cuda_stream"):
        return stream.cuda_stream
    if hasattr(stream, "sycl_queue"):
        return stream.sycl_queue
    raise RuntimeError(
        "Accelerator Sanitizer requires stream objects to expose `native_handle`."
    )


class ArgumentHandler:
    def __init__(self, device_type: str | None = None) -> None:
        self.device_type = _resolve_device_type(device_type)
    ...
    def _handle_argument(
        ...
        if isinstance(value, torch.Tensor) and value.device.type == self.device_type:
...
class AcceleratorSanitizerDispatchMode(TorchDispatchMode):
    def __init__(self, device: _device_t = None) -> None:
        self.device_type = _resolve_device_type(device)
        self.event_handler = EventHandler()
        gpu_trace = _get_gpu_trace_module(self.device_type)
        torch._C._activate_gpu_trace()
        _register_trace_callbacks(gpu_trace, self.event_handler)
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        argument_handler = ArgumentHandler(self.device_type)
        ...
        if not (argument_handler.dataptrs_read or argument_handler.dataptrs_written):
            return outputs

        errors = self.event_handler._handle_kernel_launch(
            _get_stream_id(_get_current_stream(self.device_type)),
        ...
        if errors:
            ...
            raise AcceleratorSanitizerErrors(errors)

        return outputs


class AcceleratorSanitizer:
    """Manages the lifetime of an AcceleratorSanitizer dispatch mode object.

    The AcceleratorSanitizer class wraps the entering/exiting functions of the
    dispatch mode context manager in the enable function/destructor, respectively.
    This is to explicitly set the lifetime of the dispatch mode object to that of
    the application.
    """

    def __init__(self, device: _device_t = None) -> None:
        self.device = device
        self.dispatch: AcceleratorSanitizerDispatchMode | None = None
        self.enabled = False

    def _get_dispatch(self) -> AcceleratorSanitizerDispatchMode:
        if self.dispatch is None:
            self.dispatch = AcceleratorSanitizerDispatchMode(self.device)
        return self.dispatch

    def enable(self):
        if not self.enabled:
            self._get_dispatch().__enter__()
            self.enabled = True

    def disable(self):
        if self.enabled and self.dispatch is not None:
            self.dispatch.__exit__(None, None, None)
            self.enabled = False

    def __del__(self):
        # Since this object lifetime is linked to the `torch.accelerator._sanitizer`
        # python module, it often gets deleted as part of the overall `torch` module
        # cleanup. At that time, depending on CPython version, the torch.* module
        # might be in different states of being already cleaned up. Similarly other
        # imports might already have been cleaned up so `sys` might be already gone
        # as well. Skip exiting the mode if it outlived the runtime.
        if (sys is not None) and (not sys.is_finalizing()) and self.enabled:
            self.disable()


def enable_accelerator_sanitizer():
    """Enable Accelerator Sanitizer.

    The sanitizer will begin to analyze low-level accelerator calls invoked by
    torch functions for synchronization errors. All data races found will be
    printed to the standard error output along with stack traces of suspected
    causes. For best results, the sanitizer should be enabled at the very
    beginning of the program.
    """
    accelerator_sanitizer.enable()


accelerator_sanitizer = AcceleratorSanitizer()
...
__all__ = [
    ...
    "TensorInfo",
    "UnsynchronizedAccessError",
    "accelerator_sanitizer",
    "enable_accelerator_sanitizer",
    "zip_arguments",
    "zip_by_key",
]
